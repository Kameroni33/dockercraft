#!/bin/sh
#
# Generic Minecraft server launcher (POSIX sh — runs inside the MC image).
# All provisioning (jar download, mods, server.properties) happens on the
# host via the manager — this only launches.
