...
def _server(client, loader="fabric", name="modded"):
    return client.post(
        "/api/servers", json={"name": name, "mc_version": "1.21.1", "loader": loader}
    ).json()["id"]


def test_install_resolves_dependencies(client, session, fake_modrinth):
    sid = _server(client)
    resp = client.post(f"/api/servers/{sid}/mods", json={"project": "geyser"})
    assert resp.status_code == 201
    titles = [m["title"] for m in resp.json()["installed"]]
    assert titles == ["Floodgate", "Geyser"]  # dep installed first

    inst_mods = {m["slug"]: m for m in client.get(f"/api/servers/{sid}/mods").json()}
...
def test_uninstall_dependency_protection(client, session, fake_modrinth):
    sid = _server(client)
    client.post(f"/api/servers/{sid}/mods", json={"project": "geyser"})
    resp = client.delete(f"/api/servers/{sid}/mods/FLOOD1")
    assert resp.status_code == 409 and "Geyser" in resp.json()["detail"]
    assert client.delete(f"/api/servers/{sid}/mods/FLOOD1?force=true").status_code == 204
    # Geyser itself uninstalls freely
    assert client.delete(f"/api/servers/{sid}/mods/GEYSER1").status_code == 204
    assert client.get(f"/api/servers/{sid}/mods").json() == []


def test_incompatible_and_wrong_loader(client, fake_modrinth):
    sid = _server(client)
    incompat = client.post(f"/api/servers/{sid}/mods", json={"project": "forge-only"})
    assert incompat.status_code == 422
    assert client.post(f"/api/servers/{sid}/mods", json={"project": "nope"}).status_code == 404
    vanilla = _server(client, loader="vanilla", name="plain")
    wrong = client.post(f"/api/servers/{vanilla}/mods", json={"project": "geyser"})
    assert wrong.status_code == 409


def test_toggle_enabled(client, session, fake_modrinth):
    sid = _server(client)
    client.post(f"/api/servers/{sid}/mods", json={"project": "sodium"})
    from api.services.instances import get_instance

    d = mods_dir(get_instance(session, sid))
    assert (d / "sodium-0.6.jar").exists()

    client.patch(f"/api/servers/{sid}/mods/SODIUM1", json={"enabled": False})
    assert not (d / "sodium-0.6.jar").exists()
    assert (d / "sodium-0.6.jar.disabled").exists()
    client.patch(f"/api/servers/{sid}/mods/SODIUM1", json={"enabled": True})
    assert (d / "sodium-0.6.jar").exists()


def test_update_flow(client, session, fake_modrinth):
    sid = _server(client)
    client.post(f"/api/servers/{sid}/mods", json={"project": "sodium", "version_id": "sv1"})
    updates = client.post(f"/api/servers/{sid}/mods/check-updates").json()
    assert updates == [{
        "project_id": "SODIUM1", "title": "Sodium", "installed": "0.5",
        "available": "0.6", "available_version_id": "sv2", "auto_update": False,
    }]

    resp = client.post(f"/api/servers/{sid}/mods/SODIUM1/update").json()
    assert resp["updated"] is True and resp["version"] == "0.6"
    from api.services.instances import get_instance

    d = mods_dir(get_instance(session, sid))
    assert (d / "sodium-0.6.jar").exists() and not (d / "sodium-0.5.jar").exists()
    assert client.post(f"/api/servers/{sid}/mods/check-updates").json() == []
    # Already current: no-op
    assert client.post(f"/api/servers/{sid}/mods/SODIUM1/update").json()["updated"] is False


def test_auto_update_flag(client, fake_modrinth):
    sid = _server(client)
    client.post(f"/api/servers/{sid}/mods", json={"project": "sodium", "version_id": "sv1"})
    mod = client.patch(f"/api/servers/{sid}/mods/SODIUM1", json={"auto_update": True}).json()
...
def test_auto_update_sweep(client, session, fake_modrinth):
    ...
    sid = _server(client)
    client.post(f"/api/servers/{sid}/mods", json={"project": "sodium", "version_id": "sv1"})
    client.post(f"/api/servers/{sid}/mods", json={"project": "geyser"})  # current; no auto

    assert scheduler.run_mod_updates(session) == []  # nothing flagged yet
    client.patch(f"/api/servers/{sid}/mods/SODIUM1", json={"auto_update": True})
