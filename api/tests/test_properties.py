...
def _create(client, name="props-test"):
    return client.post("/api/servers", json={"name": name, "mc_version": "1.21.1"}).json()


def test_properties_roundtrip(client):
    sid = _create(client)["id"]
    resp = client.patch(
        f"/api/servers/{sid}/properties",
    ...
    assert client.get(f"/api/servers/{sid}/properties").json()["motd"] == "Brother SMP"


def test_managed_keys_rejected(client):
    sid = _create(client)["id"]
    resp = client.patch(f"/api/servers/{sid}/properties", json={"server-port": 1234})
...
def test_survives_mc_style_rewrite(client):
    ...
    sid = _create(client)["id"]
    client.patch(f"/api/servers/{sid}/properties", json={"motd": "hi"})
    path = paths.instance_dir("props-test") / "server.properties"
    path.write_text("#Minecraft server properties\n#Sat Jun 06\nmotd=hi\nlevel-seed=\n")
    props = client.get(f"/api/servers/{sid}/properties").json()
