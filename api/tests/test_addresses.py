...
def test_addresses(client, fake_docker, monkeypatch):
    ...
    client.post("/servers", json={"name": "one", "mc_version": "1.21.1"})
    client.post("/servers", json={"name": "two", "mc_version": "1.21.1"})
    fake_docker["statuses"]["one"] = "running"

    body = client.get("/addresses").json()
...
def test_lan_ip_override(client, monkeypatch):
    ...
    assert client.get("/addresses").json()["lan_ip"] == "10.0.0.99"
