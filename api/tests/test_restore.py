...
def _server(client, name="rst"):
    return client.post(
        "/api/servers", json={"name": name, "mc_version": "1.21.1", "loader": "fabric"}
    ).json()["id"]
...
def test_restore_in_place(client, session):
    ...
    bid = client.post(f"/api/servers/{sid}/backups", json={"note": "good"}).json()["id"]

    level.write_bytes(b"CREEPER GRIEFED EVERYTHING")
    resp = client.post(f"/api/backups/{bid}/restore")
    ...
    client.post(f"/api/backups/{safety_id}/restore")
...
def test_restore_running_server_stops_and_restarts(client, session, fake_docker, fake_rcon):
    ...
    client.post(f"/api/backups/{bid}/restore")
...
def test_clone_to_new_instance(client, session):
    ...
    resp = client.post(f"/api/backups/{bid}/clone", json={"name": "rst-copy"})
    assert resp.status_code == 201
    data = resp.json()
    src = client.get(f"/api/servers/{sid}").json()
    ...
    assert client.post(f"/api/backups/{bid}/clone", json={"name": "rst-copy"}).status_code == 409
...
def test_orphan_backup_restore_409_but_clone_works(client, session):
    ...
    assert client.post(f"/api/backups/{bid}/restore").status_code == 409
    resp = client.post(f"/api/backups/{bid}/clone", json={"name": "phoenix"})
    assert resp.status_code == 201
