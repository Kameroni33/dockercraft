...
from api.services import docker_manager, ports
...
def create_instance(session: Session, data: InstanceCreate) -> ServerInstance:
    ...
    docker_manager.instance_dir(instance).mkdir(parents=True, exist_ok=True)
    return instance
...
def delete_instance(session: Session, instance: ServerInstance, delete_data: bool = False) -> None:
    ...
    if delete_data:
        shutil.rmtree(docker_manager.instance_dir(instance), ignore_errors=True)
